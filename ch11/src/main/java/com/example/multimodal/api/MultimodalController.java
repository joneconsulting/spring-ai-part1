...
public class MultimodalController {
    ...
    public Map<String, Object> generateImage(@RequestParam String prompt) {
        String url = imageGenService.generate(prompt);
        return Map.of("prompt", prompt, "url", url,
                "note", "URL 은 만료됩니다. 실무에서는 즉시 스토리지에 저장하세요.");
    }

    @PostMapping("/api/vision/describe")
    public Map<String, Object> describe(@RequestParam("file") MultipartFile file) throws IOException {
        Resource img = file.getResource();
        return Map.of("filename", file.getOriginalFilename(),
                "description", visionService.describe(img));
    }
