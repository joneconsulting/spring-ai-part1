...
public class TuningService {
    ...
    public Map<String, Object> ingestAll() {
        ...
        for (int size : chunkSizes) {
            ...
            reader.getCustomMetadata().putAll(Map.of(
                    ...
                    "category", "인사규정",
                    "chunkSize", String.valueOf(size)     // 실험 구분 키
            ));
            ...
            result.put("chunkSize_" + size, Map.of(
                    "chunks", chunks.size(),
                    "avgChars", Math.round(avgLen)));
    ...
    public Map<String, Object> search(String question, int topK, double threshold,
        ...
        if (chunkSize != null) filters.add("chunkSize == '" + chunkSize + "'");
        if (category != null && !category.isBlank()) filters.add("category == '" + category + "'");
        if (!filters.isEmpty()) req.filterExpression(String.join(" && ", filters));

        List<Document> found = vectorStore.similaritySearch(req.build());

        return Map.of(
                "question", question,
                "params", Map.of("topK", topK, "threshold", threshold,
