...
public class EvaluationService {
    ...
    public Report run() {
        ...
        for (EvaluationSet.EvalQuestion q : evaluationSet.questions()) {
            ...
            if (!q.answerable()) {
                ...
                pass = saidNotFound;
                note = pass ? "환각 억제 성공" : "근거 없이 답변함 (환각)";
            } else if (saidNotFound) {
                // 답할 수 있어야 하는데 못 찾음 -> 검색 실패
                pass = false;
                note = "검색 실패 (topK/threshold 확인 필요)";
            } else {
                // LLM-as-a-Judge 로 관련성 판정
                EvaluationRequest req =
                        new EvaluationRequest(q.question(), r.evidences(), r.answer());
                EvaluationResponse res = relevancyEvaluator.evaluate(req);
                pass = res.isPass();
                note = pass ? "관련성 통과" : "관련성 미달 (동문서답 가능성)";
            }
    ...
    private Report summarize(List<ItemResult> items) {
        ...
        long passed = items.stream().filter(ItemResult::pass).count();

        Map<String, String> byType = new LinkedHashMap<>();
        for (String type : List.of("general", "boundary", "trap")) {
            List<ItemResult> sub = items.stream().filter(i -> i.type().equals(type)).toList();
            long p = sub.stream().filter(ItemResult::pass).count();
            byType.put(type, sub.isEmpty() ? "-" : "%d/%d (%.0f%%)".formatted(p, sub.size(), pct(p, sub.size())));
        }

        double avgTokens = items.stream().mapToInt(ItemResult::contextTokensApprox).average().orElse(0);
        List<String> failed = items.stream().filter(i -> !i.pass()).map(ItemResult::id).toList();
    ...
    private static double pct(long n, long d) { return d == 0 ? 0 : n * 100.0 / d; }
