...
/** S5 프롬프트 품질 · S6 역할 구조 · S7 System Prompt 4요소 */
@RestController
...
public class PromptQualityController {
    ...
    /** S5 — Before/After: "이 기사 요약해줘" vs 역할·형식·제약 명시 */
    @GetMapping("/quality/summary")
    public BeforeAfterResponse summary(@RequestParam(required = false) String article) {
        return service.summaryBeforeAfter(
                article == null ? PromptQualityService.SAMPLE_ARTICLE : article);
    }

    /** S6 — 역할 분리(system/user)로 호출 */
    ...
    public Map<String, String> roleSeparated(
        ...
        return Map.of("question", question, "answer", service.roleSeparated(question));
    }

    /** S6 — 안티패턴 비교: 사용자 입력을 system에 섞는 경우 */
    @GetMapping("/role/anti-pattern")
    public BeforeAfterResponse antiPattern(
            @RequestParam(defaultValue = "@Transactional의 전파 속성을 설명해 주세요") String question) {
        return service.roleSeparationAntiPattern(question);
    }

    /** S7 — 4요소(역할·형식·제약·톤) 고객센터 봇 */
    ...
    /** S7 — 제약(하지 말 것)의 효과: 정책에 없는 질문을 넣어 본다 */
    ...
    public Map<String, String> outOfPolicy(
        ...
        return Map.of(
                "question", question,
                "answer", service.customerCenterBot(company, question),
                "기대 동작", "'상담원 연결을 도와드리겠습니다'로 답해야 정상");
    }
}
