...
public class DifyChatService {
    ...
    public ChatResult chat(String userId, String question) {
        ...
        String convId = sessionRepo.findConversationId(userId).orElse("");

        DifyProxyClient.DifyReply reply = difyClient.chat(question, userId, convId);

        // ② 응답의 conversation_id 를 반드시 저장 (멀티턴의 핵심)
        sessionRepo.save(userId, reply.conversationId());
    ...
    public Flux<String> chatStream(String userId, String question) {
        ...
        return difyClient.chatStream(question, userId, convId);
    }
