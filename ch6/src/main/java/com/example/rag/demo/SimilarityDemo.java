...
public class SimilarityDemo {
    ...
    public void run() {
        ...
        System.out.printf("질의: \"%s\"  (임베딩 차원 = %d)%n%n", query, queryVector.length);
        System.out.printf("%-22s %12s   %s%n", "비교 대상", "코사인유사도", "막대");
        System.out.println("-".repeat(66));

        candidates.forEach((label, text) -> {
            float[] v = embeddingModel.embed(text);
            double sim = cosineSimilarity(queryVector, v);
            System.out.printf("%-22s %12.4f   %s%n", label, sim, bar(sim));
        });
